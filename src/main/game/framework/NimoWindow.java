...
public class NimoWindow extends JFrame
{
    ...
    public void setBoardGame() {
        NimoGame game = new NimoGame(this);

    // Créer de faux joueurs pour tester
    ...
    // Initialiser les decks des joueurs
        for (Player p : players) {
            for (int i = 0; i < 7; i++) { // Distribuer 7 cartes par joueur
                CardView card = NimoWindow.createTestCard(); // fonction à créer pour générer des cartes factices
                p.getDeck().addCard(card);
            }
        }

    // Connecter les joueurs au jeu
    ...
    public static CardView createTestCard() {
        // juste pour tester visuellement
        Image img = getImage("cards/blue_5.png"); // remplace par une image valide
        return new CardView(img, 'B', 5); // Blue 5
    }
